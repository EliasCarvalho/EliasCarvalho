...
gera.pa <-function(bn.structure, data.to.work, pa.name, pa.imgname, bn.algorithm, bn.score.test, outcome.var)
{
  ...
  # Verify if endogenous variables are binary or ordered categorical and mount a list do declare it in sem command
  ##### ONLY FOR ENDOGENOUS ###
  ...
  cat ("\nRunning the Path Analysis model...\n") ################### cat
    ...
    # Creates a dataframe to store indexed (X2, df, p-value, RMSEA, SRMR, RMR, CFI, GFI, X2/DF)
    # RMSEA - < 0.08 ( Hair et al., 2006)
    # SRMR - <0.08 - (Hu and Bentler, 1999)
    # CFI - >0.90 - >0.90 (Hu and Bentler, 1999)
    # GFI - >0.90 - >0.90 ( Hair et al. 2006)
    # X2/DF - ≤ 5.00 ( Hair et al., 1998)
    # sem.indexes.names   <- c("chisq", "df", "pvalue",  "srmr", "rmsea",   "cfi",   "tli", "mfi","chisq/df")
    # sem.indexes.cutoff  <- c(     "",   "",  ">0.05", "<0.08", "<0.07", ">0.92", ">0.92",">0.92",  "<3")
    sem.indexes.names   <- c("chisq", "df", "pvalue",  "rmsea",  "srmr",   "cfi",   "gfi", "chisq/df")
    sem.indexes.cutoff  <- c(     "",   "",  ">0.05",  "<0.08", "<0.08", ">0.90", ">0.90",       "<3")
    ...
    writexl::write_xlsx(df.sem.indexes, sem.index.name)
    ...
    pa.resname <- paste("docPA_Res_", bn.algorithm, "_", bn.score.test,"_", nrow(data.to.work), ".xls", sep = "")
    ...
    grDevices::dev.off()
    # Calculate the model residuals and export it
    pa.residuals <- lavaan::residuals(pa.model.fit, type = "cor")
    df.residuals <- as.data.frame(pa.residuals$res.cov)
    
    writexl::write_xlsx(df.residuals, pa.resname)
