...
check.package <- function(package.name)
{
  if (!is.element(package.name, utils::installed.packages()))
  {
    ...
    {
      commandAssign <- paste("if (!requireNamespace('BiocManager', quietly = TRUE))  install.packages('BiocManager')", sep = "")
      eval(parse(text=commandAssign))

      commandAssign <- paste("BiocManager::install(c('graph', 'RBGL', 'Rgraphviz'))", sep = "")
      eval(parse(text=commandAssign))
    ...
    else
    {
      ...
      {
        commandAssign <- paste("library(",package.name,")", sep = "")
        eval(parse(text=commandAssign))
      } # else
    } # else
  ...
  else
  {
    commandAssign <- paste("library(",package.name,")", sep = "")
    eval(parse(text=commandAssign))
  } # else # if (!is.element(package.name, installed.packages()))
} # check.package <- function(package.name)
